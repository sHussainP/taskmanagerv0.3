...
describe('AddTask Function Test', () => {
  ...
  it('should create a new task successfully', async () => {
    ...
    // Assertionss
    // Verifies whether a certain condition is true during the execution of a test. 
    // It ensures that the code behaves as expected. 
    expect(createStub.calledOnceWith({ userId: req.user.id, ...req.body })).to.be.true;
    expect(res.status.calledWith(201)).to.be.true;
    expect(res.json.calledWith(createdTask)).to.be.true;

    // Restore stubbed methods 
    // Is used to clean up or reset any mocked or stubbed methods or functions after a test case has run.
    createStub.restore();
  });
